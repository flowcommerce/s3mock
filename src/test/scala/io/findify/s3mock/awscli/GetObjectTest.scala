  ...
  override def behaviour(fixture: => Fixture) = {
    ...
    it should "receive LastModified header with AWS CLI" in {
      ...
      val response = Await.result(
        http.singleRequest(HttpRequest(method = HttpMethods.GET, uri = s"http://127.0.0.1:$port/awscli-lm/foo")),
        10.seconds
      )
    ...
    it should "deal with HEAD requests with AWS CLI" ignore {
      ...
      val response = Await.result(
        http.singleRequest(HttpRequest(method = HttpMethods.HEAD, uri = s"http://127.0.0.1:$port/awscli-head/foo2")),
        10.seconds
      )
      response.headers.find(_.is("last-modified")).map(_.value()) shouldBe Some("Thu, 01 Jan 1970 00:00:00 GMT")
      response.entity.contentLengthOption shouldBe Some(3)
      Await
        .result(response.entity.dataBytes.fold(ByteString(""))(_ ++ _).runWith(Sink.head), 10.seconds)
        .utf8String shouldBe ""
    }
